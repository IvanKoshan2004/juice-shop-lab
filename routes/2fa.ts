...
async function verify(req: Request, res: Response) {
  ...
  try {
    const { userId, type } =
      security.verify(tmpToken) && security.decode(tmpToken);
    ...
    challengeUtils.solveIf(
      challenges.twoFactorAuthUnsafeSecretStorageChallenge,
      () => {
        return (
          user.email === "wurstbrot@" + config.get<string>("application.domain")
        );
      }
    );
...
async function setup(req: Request, res: Response) {
  try {
    ...
    security.authenticatedUsers.updateFrom(
      req,
      utils.queryResultToJson(userModel)
    );
